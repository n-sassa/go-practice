...
import (
	...
	"swagger/server/clients"
	"swagger/server/gen/restapi/factory"
	"swagger/server/gen/restapi/factory/user"
)

//go:generate swagger generate server --target ../../gen --name Factory --spec ../../../swagger/swagger.yaml --api-package factory --principal interface{} --exclude-main
...
func configureAPI(api *factory.FactoryAPI) http.Handler {
	...
	dbClient.AutoMigrate(&clients.Customer{})

	newUser := clients.Customer{FirstName: "test", LastName: "太郎", PhoneNo: 1111111111, Password: "test", Email: "test@gmail.com"}
	result := dbClient.Create(&newUser)
	fmt.Println(result)
	data := []clients.Customer{}
	dbClient.Table("customer").Find(&data)
